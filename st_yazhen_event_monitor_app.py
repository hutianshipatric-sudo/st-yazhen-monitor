...
def fetch_spot(symbol: str) -> Tuple[pd.DataFrame, Dict]:
    ...
    try:
        ...
        if row.empty:
            return df, {"error": f"实时列表中未找到代码 {symbol}"}

        return df, row.iloc[0].to_dict()
...
def add_daily_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 量能 / 换手分位
    # 这里用 iloc，避免新版 Pandas 把 x[-1] 理解成索引标签导致 KeyError。
    out["AMT_PCTL_60"] = out["成交额"].rolling(60).apply(
        lambda x: percentile_rank(x.iloc[:-1], x.iloc[-1]) if len(x) > 10 else np.nan,
        raw=False,
    )

    if "换手率" in out.columns:
        out["TURN_PCTL_60"] = out["换手率"].rolling(60).apply(
            lambda x: percentile_rank(x.iloc[:-1], x.iloc[-1]) if len(x) > 10 else np.nan,
            raw=False,
        )
...
def add_minute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "成交额" in out.columns and "成交量" in out.columns:
        out["VWAP"] = out["成交额"].cumsum() / out["成交量"].replace(0, np.nan).cumsum()
    else:
        out["VWAP"] = np.nan
...
def calculate_scores(
    ...
    ma10 = _to_float(last.get("MA10"))
    ma20 = _to_float(last.get("MA20"))
    rsi = _to_float(last.get("RSI14"))
    macd = _to_float(last.get("MACD_HIST"))
    ...
    if price > ma5 > ma10:
        long_score += 10
        scores["reasons_long"].append("价格站上 MA5/MA10，短线趋势偏强")

    if price > ma20:
        long_score += 5
        scores["reasons_long"].append("价格位于 MA20 上方，中短期趋势未破")

    if macd > 0:
        long_score += 7
        scores["reasons_long"].append("MACD 柱为正，动量仍在")

    if 50 <= rsi <= 75:
        long_score += 8
        scores["reasons_long"].append("RSI 处于强势但未极端过热区")
    elif rsi > 85:
        long_score -= 10
        scores["risk_flags"].append("RSI 极端过热，追高风险上升")

    if not pd.isna(amt_pct) and amt_pct >= 70:
        long_score += 8
        scores["reasons_long"].append("成交额处于近 60 日高分位，资金活跃")

    if not pd.isna(vol_ratio) and vol_ratio >= 1.2:
        long_score += 5
        scores["reasons_long"].append("量比放大，事件资金正在参与")

    if 0 < pct_chg < limit_pct_now * 0.85:
        long_score += 5
        scores["reasons_long"].append("上涨但未逼近涨停，承接相对健康")

    if pct_chg >= limit_pct_now * 0.9:
        long_score -= 7
        scores["risk_flags"].append("接近涨停/高开极限，T+1 买入后隔夜风险较高")

    if ret10 > 35:
        long_score -= 8
        scores["risk_flags"].append("10 日涨幅过大，利好兑现风险上升")
    ...
    if rsi > 80:
        short_score += 10
        scores["reasons_short"].append("RSI 高位，存在兑现压力")

    if ret5 > 20 or ret10 > 35:
        short_score += 10
        scores["reasons_short"].append("短期累计涨幅过大，容易出现利好兑现")

    if not pd.isna(amt_pct) and amt_pct >= 85 and pct_chg < 2:
        short_score += 8
        scores["reasons_short"].append("成交额高分位但价格不强，疑似放量滞涨")

    if macd < 0:
        short_score += 8
        scores["reasons_short"].append("MACD 柱转弱，动量衰减")

    if price < ma5:
        short_score += 7
        scores["reasons_short"].append("跌破 MA5，短线趋势破坏")
    ...
    if atr_pct > 5:
        risk_score += 15
        scores["risk_flags"].append("ATR% 较高，单日波动容错低")
    elif atr_pct > 3:
        risk_score += 8
        scores["risk_flags"].append("ATR% 偏高，需要缩小仓位")

    if vol20 > 80:
        risk_score += 12
        scores["risk_flags"].append("20日年化波动率过高")
    ...
    if days_to_event in [0, 1, 2]:
        risk_score += 10
        scores["risk_flags"].append("事件临近，波动和兑现风险同步升高")
    ...
    if minute is not None and not minute.empty and len(minute) >= 30:
        ...
        first_open = _to_float(minute.iloc[0].get("开盘", mprice))
        intraday_ret = (mprice / first_open - 1) * 100 if first_open > 0 else np.nan

        if not pd.isna(vwap) and mprice > vwap:
            long_score += 7
            intraday_note.append("分钟价格站上 VWAP，日内承接强")
        elif not pd.isna(vwap) and mprice < vwap:
            long_score -= 8
            short_score += 8
            intraday_note.append("分钟价格跌破 VWAP，日内承接弱")

        if not pd.isna(mma5) and not pd.isna(mma20) and mprice > mma5 > mma20:
            long_score += 5
            intraday_note.append("5分钟均线强于20分钟均线，日内动量向上")

        if not pd.isna(intraday_ret) and intraday_ret > limit_pct_now * 0.75:
            risk_score += 7
            intraday_note.append("日内涨幅接近涨停区，追高风险增加")
...
def monte_carlo_forecast(
    ...
    horizon_days: int,
    n_sims: int,
    event_jump_mean: float,
    event_jump_std: float,
) -> pd.DataFrame:
...
with st.sidebar:
    ...
    default_event = next_weekday(datetime.now().date(), 2)
...
if not daily.empty and len(daily) > 60:
    ...
    if not pd.isna(price):
        ...
        st.write(
            f"**T+1隔夜压力测试：** 若买入后不能当天卖出，下一交易日理论下跌限制按 "
            f"{limit_pct_after:.1f}% 估算，不利价格约为 **{down_limit_after:.2f}**。"
            f"若仍按当前 ST 限制 {limit_pct_now:.1f}% 估算，不利价格约为 **{down_limit_now:.2f}**。"
        )
else:
